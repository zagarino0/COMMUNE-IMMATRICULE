...
function InfoContribuableRadie() {
    let navigate = useNavigate();
    const selectedData = localStorage.getItem("selectedContribuableRadieData");
    const  parsedDataSelected = JSON.parse(selectedData as string);
    console.log("Data from Radie :",parsedDataSelected)
    ...
    const HandleContribuableRadie = async () => {
      ...
      try {
        ...
        alert(`Mise en radié pour ${parsedDataSelected.raison_social} réussi`)
        navigate('/RelanceDeffaillant')
      } catch (error) {
        // Handle errors
...
export default InfoContribuableRadie